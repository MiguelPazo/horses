...
SELECT a.id, a.code, a.name, DATE_FORMAT(a.birthdate, '%d-%m-%Y') AS birthdate, c.prefix, CONCAT(c.names, ' ', c.lastnames) AS breeder, 
CONCAT(e.names, ' ', e.lastnames) AS 'owner', mom.prefix AS mom_prefix, mom.name AS mom_name, dad.prefix AS dad_prefix, dad.name AS dad_name
FROM animals a
